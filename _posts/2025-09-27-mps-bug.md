...
toc:
  ...
  - name: Finding the Broken Operations
    subsections:
      ...
      - name: How Two Bugs Masked Each Other
...
I had a shockingly fun time deep-diving this, learned a lot, and wrote up the whole journey. So if you enjoy debugging mysteries or find that tracking down bugs teaches you more than documentation ever could, this might resonate. We go through the investigation step-by-step, explaining the framework internals as they become necessary to crack the case 🕵️‍♀️

_Also debugging post-mortems sometimes make me worry I'd wouldn't have been smart enough / had enough background knowledge to do figure them out on my own, so I tried to make this accessible and explain as we go, as I really think any ML eng could have done this if curious / stubborn enough._

<details>
<summary><b>TL;DR - Just tell me the bug</b></summary>
<div>
  <p><strong>The Bug:</strong> PyTorch's MPS (Apple Silicon GPU) backend has a kernel bug where <code>addcmul_</code> and <code>addcdiv_</code> operations silently fail when writing to non-contiguous output tensors.</p>

  <p><strong>Why It Cause the Training Plateau:</strong></p>
  <ul>
    <li>Encoder weights initialized as transpose of decoder &rarr; non-contiguous memory layout</li>
    <li>Adam's state tensors inherited this layout (<code>exp_avg_sq</code> and <code>exp_avg_sq</code> became non-contiguous)</li>
    ...
    <li>For the non-contiguous encoder's Adam parameters, the operation for <code>exp_avg_sq</code> doesn't return the output &rarr; value stays zero but then the parameter update gets divides by this zero and <it>should</it> cause a massive update but instead just does no update because it also uses an operator that breaks with non-contiguous outputs </li>
  </ul>
...
The architecture itself is straightforward—a two-layer sparse autoencoder (encoder → sparse hidden layer → decoder) <d-footnote>This is a sparse autoencoder with tied weights - the encoder is initialized as the decoder's transpose. It has some unusual training quirks (TopK sparsity, gradient manipulation, auxiliary losses) but none that should cause this specific issue.</d-footnote>. Basic enough that I could actually inspect what each layer was doing.
...
The encoder gradients were there—and they were huge (1e7)! Yes, they were sparse (95% zeros) because of how the TopK sparsity works, but there were still plenty of non-zero gradients. So gradients are definitely flowing.
...
<ol>
  <li>
    <b>First moment (<code>exp_avg</code>)</b>: Running average of gradients – "which direction have we been consistently moving?"
  </li>
  <li>
    <b>Second moment (<code>exp_avg_sq</code>)</b>: Running average of squared gradients – "how much do gradients vary in this direction?"
  </li>
</ol>
...
# Each training step:
exp_avg = β₁ * exp_avg + (1 - β₁) * grad              # Update momentum
exp_avg_sq = β₂ * exp_avg_sq + (1 - β₂) * grad²       # Update variance

# Bias correction (compensate for zero initialization)
# Early steps: moments are biased toward zero since they start at 0
# Correction: divide by (1 - β^step), which starts small and → 1
exp_avg_corrected = exp_avg / (1 - β₁^step)
exp_avg_sq_corrected = exp_avg_sq / (1 - β₂^step)
step += 1
...
<ul>
  <li>
    <b>First moment (<code>exp_avg</code>)</b>: Smooths out noisy gradients by averaging recent directions – like momentum in physics. Default β₁=0.9 means "90% of last step + 10% new gradient"
  </li>
  <li>
  ...
  <li>
    <b>Bias correction</b>: Since both moments start at zero, their early values are biased downward. The correction factor <code>(1 - β^step)</code> compensates for this: at step 1 it's small (large correction), and it approaches 1 as training progresses (no correction needed) (so it is effectively making the beta values vary over time)
  </li>
  <li>
    <b>Epsilon (ε=1e-8)</b>: Prevents division by zero
  </li>
</ul>
...
```
param_update = lr × exp_avg / (√exp_avg_sq + ε)
             = 0.001 × 3.45e5 / (√0 + 1e-8)
             = 345 / 1e-8
             = 3.45e10  ← HUGE!
```
...
I tested with different beta values, at different steps, and even β₂=0 (which bypasses the exponential average entirely, making `exp_avg_sq = grad²` directly). The encoder's `exp_avg_sq` still stayed zero, making bias correction seem less likely as a culprit.
...
I know you're probably not reading this for the mental rabbit holes but honestly if a debugging adventure didn't have a single moment of ~spiraling and losing your sense of confidence/sanity~, did it even happen? Also feels disingenuous to not include this part. And hey, maybe one of these theories could've been correct so was worth considering! And for the most part LLMs definitely do know more about Adam than I do (although they're clearly less calibrated about their technical confusions).
...
<div style="
  ...
  font-weight: 500;
">
It wasn't precision at all - it's device-specific! The same float32 code works on CPU but fails on MPS.
</div>

﹡ This is progress!!

﹡  Note to self... have more confidence when debugging & stop letting LLMs confuse you
...
In my case, because I'm running this on my Mac laptop, I'm using MPS (Metal Performance Shaders), which is the GPU backend for Apple Silicon. While it feels a bit crazy to assume that my training plateau is due to a kernel bug, it's a bit less unsurprising on MPS as it’s newer and less mature than the CPU and CUDA backends. (And honestly, most people training/debugging ML models are not doing it on their macbooks.)
...
PyTorch reuses the same memory with different stride information - no data is moved! For more examples and an interactive stride visualizer, check out [ezyang's stride tutorial](https://blog.ezyang.com/2019/05/pytorch-internals/).
...
..But wait, I thought the whole point of keeping tensors non-contiguous is to save time by _avoiding_ cloning... if we're already cloning why would we ever want to preserve a non-contigous layout?!
...
### How Is Adam Implemented in PyTorch?
...
*First moment update:*
```python
exp_avg.lerp_(grad, 1-β₁)  # ✓ Works fine
```

*Second moment update:*
```python
exp_avg_sq.mul_(β₂)                        # ✓ Works fine
exp_avg_sq.addcmul_(grad, grad, 1-β₂)      # ✗ No update - stays zero!
```

**Step 4: Parameter Update**
```python
# Should update param, does nothing, leading to silent failure
param.addcdiv_(exp_avg, denom, value=-step_size)  # ✗ No update - stays zero!
```

**Result:** Everyone's favorite bug! A complete parameter freeze with no error messages 🙃

### How Two Bugs Masked Each Other

Note that the two MPS bugs compound perfectly to create a silent freeze:

**If ONLY `exp_avg_sq.addcmul_()` failed:**
- `exp_avg_sq` stays zero → update = lr * exp_avg / sqrt(ε) → HUGE updates
- Weights would explode - we'd notice immediately!

**If ONLY `param.addcdiv_()` failed:**
- `exp_avg_sq` updates correctly, `exp_avg` updates correctly
- But parameter doesn't update
- We'd see changing optimizer state but frozen weights - confusing but noticeable

**Both bugs together:**
- `exp_avg_sq` stays zero (bug 1) AND parameter doesn't update (bug 2)
- Complete silent freeze - looks like no learning is happening
...
And as we discussed earlier, PyTorch dispatches based on tensor metadata, so there isn't just _one_ implementation - there are device-specific kernels for CPU, CUDA, MPS, etc. Since my install PyTorch just has the compiled _FILL OUT_ files, to investigate the actual implementations, we need to clone PyTorch's repository.

All kernels are listed in an **operation registry** - a YAML file that maps operation names (like `addcmul_`) to their tensor-specific C++ implementations. In practice, when you compile torch (normally done before you install it), this registry is used to automatically generate hundreds of scripts that do the actual dispatching based on the patterns described here, but if we just want to understand what kernel our tensor is calling, we can look through the registry.
...
The first thing I notice is that `addc_mul_div_out_mps` and `addc_mul_div_out_mp` are both calling the same function (`addc_mul_div_out_mps`) with different args, which explains why both are broken.

Here's are some snippets of code that highlights how the output tensor is handled:
...
I don't see any particular handling of memory layouts in this code. If we trigger the early returns, that would explain the lack of output, but seem unlikely given the fact that non-contiguous tensors take longer than contiguous. There are a number of pieces in the code I'm unfamiliar with that could contain the bug (Placeholders, their MPSGraph that executes their models, how the outputs get added to the results dictionary, etc.) so instead of deep-diving all of them, let's just compare this kernel to one that we _know_ works with non-contiguous outputs, `mul_`.

### Comparing to a working implementation
The registry shows `mul_` is implemented by `binaryOpTensor` in [BinaryOps.mm](https://github.com/pytorch/pytorch/blob/v2.2.1/aten/src/ATen/native/mps/operations/BinaryOps.mm).<d-footnote>Technically it gets dispatched to mul_mps_out which then just calls the BinaryOps helper funciton.</d-footnote>
...
# But don't cargo-cult it! (TODO I hate this phrase, fix it or trim this)
...
**Silent failures are brutal to debug.** No errors, no NaNs, just weights that don't move. If you want to disentangle the effects of your hyperparameter / architecture from actual code bugs -- change up the device, dtype, code versions ASAP and see if anything changes.
...
**Clarify & simplify ASAP.** This one I knew but felt so important here, there were _so_ many potential culprits (some I skipped here for the sake of time) things got so much easier once I found a specific output that tells me if the bug was happening or not (i.e. `exp_avg_sq`=0 is so much easier to debug than _idk the loss is plateauing?_) and then it was easier to remove components from my training and see which were necessary to trigger the bug.

**Ask for LLM help with specifically scoped problems** Add some note on how when I tried asking an LLM to just debug the whole thing for me I go _nowhere_ and kept getting responses like 'it just seems like an inconsistent bug! maybe numerical instabilities' and it just generally made me question myself.. but on the flip-side when I asked for help explaining the kernel code or asking questions about PyTorch LLMs were _super_ helpful- they couldn't figure out the bug but once I understood it, they were very helpful at implementing / testing the fix (the fix was easy but knowing how to test it locally was not!) so this really reminded me to constantly calibrate to what types of tasks they are/aren't currently helpful with (this was all done pre-Sonnet-4.5 so maybe that one would have handled it better). When I tried asking details about what was breaking in the MPS code I also was just send down incorrect rabbit holes, and if the LLMs response was confident but vauge, instead of assuming I'm just confused, I should remember _it_ may be confused.

**Keep up to date** In my repo, I had another package that required an earlier version PyTorch so I hadn't prioritized keeping everything up to date, it's also just scary to be updating everything mid-project-- but since then I've just prioritized making changes that make it lower effort to quickly update (i.e. minimize dependencies with fixed versions, have lots of tests so you know if upgrading changes things, etc.)
...
**Post-mortem your bugs.** Forcing myself to re-tell the story of how i explored this and why i tried each thing I did has been honestly just as educational as the first process of doing it in real time. It really forces you to go over the logical steps of 'when you are in situation A, here are the hypothesis worth considering and how to test them' and even if you dont encounter them exactly again its SO helpful in case you do or even jsut for other debugigng. Think of it like RL, you try a . bunch of debuggign experiments, some are a bad idea, some are a good idea. Forcing yourself to explain the good ideas is like reinforcing he roll-outs that succeeded. Even if you just write it for yourself it'sv helpful, although this is the first time i've written a post-mortem for anyone but myself to read, and I *do* think it lead to more learning

-----

Anyway- if you made it this far, thanks for joining! Hope ya had fun and/or learned something & happy debugging 💜