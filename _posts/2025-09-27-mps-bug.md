...
**The Fix:** Make weights contiguous at initialization, or upgrade to PyTorch ≥2.4

**Reproduction:** A minimal reproduction script is available at [GitHub link to be added]
...
The architecture itself is straightforward—a two-layer sparse autoencoder (encoder → sparse hidden layer → decoder), <d-footnote>This is a sparse autoencoder with tied weights - the encoder is initialized as the decoder's transpose. It has some unusual training quirks (TopK sparsity, gradient manipulation, auxiliary losses) but none that should cause this specific issue.</d-footnote>. Small enough that I was training on my M3 MacBook Pro and simple enough I could actually inspect what each layer was doing.
...
| **exp_avg_sq** | <span style="display: inline-block; border: 2px solid var(--global-theme-color); border-radius: 4px; padding: 1px 10px; font-weight: bold;">0</span>           | 4.45e+12    |
...
{% include question_box.liquid emoji="💡" content="It wasn't precision at all - it's device-specific! The same float32 code works on CPU but fails on MPS." %}
...
The forward and backward passes for `F.linear` use `matmul` for their mat muls, which appear to handle non-contiguous tensors correctly on MPS. The bug is specific to the fused operations that Adam uses: addcmul_ and addcdiv_.
TODO: Actually test this claim by running F.linear with non-contiguous weights on MPS.
</div>
</details>
...
In the process of testing this locally, I had to build PyTorch locally which I'd never done before and also involved some fun learnings (but this post is too long already so I'm cutting myself off). Anyway, this process made me realize I was working on an older PyTorch (v2.2.1), old enough that I actually had to downgrade CMake in order to build it locally 😬 I now realized I probably should've upgraded by PyTorch version to see if a newer version had already fixed my bug... and turns out an ML eng at Apple had fixed this exact bug in an very similar way to what I did <d-footnote> The official fix uses `at::empty_like(self, MemoryFormat::Contiguous)` instead of `at::empty()` to create the temporary buffer, and checks contiguity using a `needsGather()` helper instead of `!is_contiguous()`. The variable naming is also reversed (output_ is the temp, output is original), and it uses a different runMPSGraph API signature. However, the core pattern is identical: detect non-contiguous output, create a contiguous temporary buffer, perform the computation, then copy results back to the original tensor.</d-footnote>. Apparently Apple even added an optimization for macOS 15+ where Metal now handles non-contiguous tensors natively!
...
<aside>Also in the process of stress-testing my kernel, I stumbled upon another niche MPS kernel bug that is actually still present in the most recent PyTorch build (2.8.1) so ended up making a PR for that & getting some use out of my new PyTorch skills</aside>
...
Anyway- if you made it this far, thanks for joining! Hope you had fun and/or learned something & happy debugging 💜